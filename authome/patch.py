...
from .cache import get_usercache,cache
...
if settings.USER_CACHE_ALIAS:
    def _get_user(request,userid=None):
        """
        ...
        user = None
        try:
            userid = userid or auth._get_user_session_key(request)
...
if settings.CACHE_USER_IN_MEMORY:
    def _get_cached_user(request):
        user = None
        try:
            userid = auth._get_user_session_key(request)
            user = cache.get_user(userid)
            if not user:
                user = _get_user(request,userid)
                if user != anonymoususer :
                    seconds = get_usercache(userid).ttl(settings.GET_USER_KEY(userid))
                    cache.set_user(user,timezone.now() + utils.get_timedelta(seconds))
        except KeyError:
            pass

        return user or anonymoususer

    auth.get_user = _get_cached_user
else:
